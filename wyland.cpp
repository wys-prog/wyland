  ...
  */
  ...
  /* III - Instructions
    ...
    */
    ...
    void load(const uir_t &unpacked) {
      switch (unpacked.vt) {
        ...
        case 0: // Value isn't embeeded.
          switch (unpacked.is) {
            case 8:  r8[unpacked.rv]  = bytemanip::from_bin<uint8_t>(read(1, r64[63])); break;
            case 16: r16[unpacked.rv] = bytemanip::from_bin<uint16_t>(read(2, r64[63])); break;
            case 32: r32[unpacked.rv] = bytemanip::from_bin<uint32_t>(read(4, r64[63])); break;
            case 64: r64[unpacked.rv] = bytemanip::from_bin<uint64_t>(read(8, r64[63])); break;
            default: throw std::runtime_error("Invalid operand size."); break;
          }
          break;
    ...
    void opRIMM(const uir_t &unpacked, _fnOP &fn) {
      switch (unpacked.is) {
        case 8:  op::anyme(r8[unpacked.rv], (uint8_t&)unpacked.vv, fn); break;
        case 16: op::anyme(r16[unpacked.rv], (uint16_t&)unpacked.vv, fn); break;
        case 32: op::anyme(r32[unpacked.rv], (uint32_t&)unpacked.vv, fn); break;
        case 64: op::anyme(r64[unpacked.rv], (uint64_t&)unpacked.vv, fn); break;
        default: throw std::runtime_error("Invalid size."); break;
      }
    }
    ...
    void opIMMIMM(const uir_t &unpacked, _fnOP &fn) {
      op::anyme((uint64_t&)unpacked.vv, (uint64_t&)unpacked.rv, fn);
    }
    ...
    void jmp(const uir_t&) {
      r64[63] = bytemanip::from_bin<uint64_t>(read(8, r64[63]));
    }
  
    void je(const uir_t&) {
      auto next = bytemanip::from_bin<uint64_t>(read(8, r64[63]));
  ...
  public:
    ...
    void initialize_instruction_set() {
      ...
      instruction_set[i_load] = [this](const uir_t &unpacked) { this->load(unpacked); };
      instruction_set[i_store] = [this](const uir_t &unpacked) { this->store(unpacked); };
      instruction_set[i_lea] = [this](const uir_t &unpacked) { this->lea(unpacked); };
      instruction_set[i_mov] = [this](const uir_t &unpacked) { this->mov(unpacked); };
      instruction_set[i_add] = [this](const uir_t &unpacked) { this->add(unpacked); };
      instruction_set[i_sub] = [this](const uir_t &unpacked) { this->sub(unpacked); };
      instruction_set[i_mul] = [this](const uir_t &unpacked) { this->mul(unpacked); };
      instruction_set[i_div] = [this](const uir_t &unpacked) { this->_div(unpacked); };
      instruction_set[i_cmp] = [this](const uir_t &unpacked) { this->cmp(unpacked); };
      instruction_set[i_jmp] = [this](const uir_t &unpacked) { this->jmp(unpacked); };
      instruction_set[i_je] = [this](const uir_t &unpacked) { this->je(unpacked); };
      instruction_set[i_jne] = [this](const uir_t &unpacked) { this->jne(unpacked); };
      instruction_set[i_jg] = [this](const uir_t &unpacked) { this->jg(unpacked); };
      instruction_set[i_jl] = [this](const uir_t &unpacked) { this->jl(unpacked); };
      instruction_set[i_jge] = [this](const uir_t &unpacked) { this->jge(unpacked); };
      instruction_set[i_jle] = [this](const uir_t &unpacked) { this->jle(unpacked); };
      instruction_set[i_jz] = [this](const uir_t &unpacked) { this->jz(unpacked); };
      instruction_set[i_jnz] = [this](const uir_t &unpacked) { this->jnz(unpacked); };
      instruction_set[i_xor] = [this](const uir_t &unpacked) { this->xor_op(unpacked); };
      instruction_set[i_or] = [this](const uir_t &unpacked) { this->or_op(unpacked); };
      instruction_set[i_and] = [this](const uir_t &unpacked) { this->and_op(unpacked); };
      instruction_set[i_int] = [this](const uir_t &unpacked) { this->_int(unpacked); };
      instruction_set[i_call] = [this](const uir_t &unpacked) { this->call(unpacked); };
      instruction_set[i_ret] = [this](const uir_t &unpacked) { this->ret(unpacked); };
      instruction_set[i_halt] = [this](const uir_t&) { flags_halt = true; };
    }
  ...
  /* 5 - Parsing arguments 
    Okay, so we need to handle arguments.
    Each arguments are simply a path for a file.
    We'll open these file in binary mode, and if we have 
    free memory space, load the binary code into memory. 
